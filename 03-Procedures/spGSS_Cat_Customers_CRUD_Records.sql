USE [DBQS]
GO
/****** Object:  StoredProcedure [dbo].[spGSS_Cat_Customers_CRUD_Records]    Script Date: 9/18/2025 9:55:37 PM ******/
SET ANSI_NULLS ON
GO
...
Desc:		GSS_Cat_Customers | Create - Read - Upadate - Delete 
Date:		09/18/2025
Example:
			spGSS_Cat_Customers_CRUD_Records @pvOptionCRUD		= 'C', 
                                        ...
                                        @pvUser                 = 'AZEPEDA', 
                                        @pvIP                   ='192.168.1.254'
			spGSS_Cat_Customers_CRUD_Records @pvOptionCRUD = 'R', @piIdCustomer	= 681, @pvUser = 'ALZEPEDA'
			
			spGSS_Cat_Customers_CRUD_Records @pvOptionCRUD = 'R', @piIdCustomer	= 681,  @pvIdCountry = 'MX', @pvIdCustomerType = 'BILL'

			SELECT * FROM GSS_Cat_Customers

			spGSS_Cat_Customers_CRUD_Records @pvOptionCRUD = 'R', 
										@pvIdLanguageUser = 'SPA',
										@pvUser = 'ALZEPEDA'
			
			spGSS_Cat_Customers_CRUD_Records @pvOptionCRUD = 'U', 
...
*/
CREATE PROCEDURE [dbo].[spGSS_Cat_Customers_CRUD_Records]
...
BEGIN TRY
	...
	DECLARE @vDescription	Varchar(255)	= 'GSS_Cat_Customers - ' + @vDescOperationCRUD 
	DECLARE @bSuccessful	Bit				= 1	
	DECLARE @vMessageType	Varchar(30)		= dbo.fnGetTransacMessages('OK',@pvIdLanguageUser)	--success
	DECLARE @vMessage		Varchar(Max)	= dbo.fnGetTransacMessages(@vDescOperationCRUD,@pvIdLanguageUser)	
	DECLARE @vExecCommand	Varchar(Max)	= "EXEC spGSS_Cat_Customers_CRUD_Records @pvOptionCRUD =  '" + ISNULL(@pvOptionCRUD,'NULL') + "', 
	...
	BEGIN
		...
		IF EXISTS(SELECT * FROM GSS_Cat_Customers WHERE Id_Customer = @piIdCustomer AND Id_Customer_Type = @pvIdCountry AND Id_Country = @pvIdCustomerType )
		BEGIN
			SET @bSuccessful	= 0
		...
		ELSE -- Don�t Exists
		BEGIN
		SET @piIdCustomer = (SELECT ISNULL(MAX(Id_Customer),0) + 1 FROM GSS_Cat_Customers )
			INSERT INTO GSS_Cat_Customers (
				Id_Customer,
				Id_Country,
				Id_Customer_Type,
				[Name],
