...
-- vwQuotation
/* ==================================================================================*/	
PRINT 'Crea View: vwQuotation'

IF OBJECT_ID('vwQuotation','V') IS NOT NULL
       DROP VIEW [dbo].vwQuotation
GO
...
FROM	Quotation WITH(NOLOCK)
		...
		Quotation.Id_Customer_Type_Bill_To = Cat_Customers_Type.Id_Customer_Type 
		...
		Quotation.Id_Customer_Type_Final = Cat_Customers_Type_Final.Id_Customer_Type 
		...
		Quotation.Id_Language = Cat_Languages.Id_Language 

		INNER JOIN Cat_Plugs WITH(NOLOCK) ON
		Quotation.Id_Plug = Cat_Plugs.Id_Plug 

		INNER JOIN Cat_Incoterm ON
		Quotation.Id_Incoterm = Cat_Incoterm.Id_Incoterm 

		INNER JOIN Cat_Currencies WITH(NOLOCK) ON
		Quotation.Id_Currency = Cat_Currencies.Id_Currency 
		...
		Quotation.Id_Sales_Type = Cat_Sales_Types.Id_Sales_Type 

		INNER JOIN Cat_Prices_Lists WITH(NOLOCK) ON
		Quotation.Id_Price_List = Cat_Prices_Lists.Id_Price_List 
		...
		Quotation.Id_Quotation_Status = Cat_Quotation_Status.Id_Quotation_Status 

		INNER JOIN Cat_Validity_Prices WITH(NOLOCK) ON
		Cat_Validity_Prices.Id_Validity_Price = Quotation.Id_Validity_Price

		LEFT OUTER JOIN Cat_Languages AS Cat_Languages_Translation WITH(NOLOCK) ON
		Quotation.Id_Language_Translation = Cat_Languages_Translation.Id_Language 