...
/* ==================================================================================*/
-- spQuotation_ApprovalRoutes_Ins_Workflow_Quotation_Header
/* ==================================================================================*/	
PRINT 'Crea Procedure: spQuotation_ApprovalRoutes_Ins_Workflow_Quotation_Header'

IF OBJECT_ID('[dbo].[spQuotation_ApprovalRoutes_Ins_Workflow_Quotation_Header]','P') IS NOT NULL
       DROP PROCEDURE [dbo].spQuotation_ApprovalRoutes_Ins_Workflow_Quotation_Header
GO
...
CREATE PROCEDURE [dbo].spQuotation_ApprovalRoutes_Ins_Workflow_Quotation_Header
...
BEGIN TRY
	...
	DECLARE @vExecCommand	Varchar(Max)	= "EXEC spQuotation_ApprovalRoutes_Ins_Workflow_Quotation_Header  @pvIdLanguageUser = '" + ISNULL(@pvIdLanguageUser,'NULL') + "', @piFolio = '" + ISNULL(CAST(@piFolio AS VARCHAR),'NULL') + "', @piVersion = '" + ISNULL(CAST(@piVersion AS VARCHAR),'NULL') + "', @pvUser = '" + ISNULL(@pvUser,'NULL') + "', @pvIP = '" + ISNULL(@pvIP,'NULL') + "'"
	...
	BEGIN
		...
		BEGIN
			...
			Q.Id_Country_Bill_To = ZC.Id_Country AND 
			ZC.[Status] = 1
			...
			SELECT 
					...
					QH.Id_Header,
					QH.Item_Template,
					Id_Approval_Section = 1, -- Quotation Header
					Id_Approval_Type	= 1, -- Discount
			...
			INSERT INTO Approval_Workflow(
				...
				SELECT DISTINCT
					...
					QH.Item_Template,
					Id_Approval_Section = 1, -- Quotation Header
					Id_Approval_Type = 2, -- Extended Warranty
					APT.Id_Approval_Flow,
					APT.Id_Role,
					Id_Approval_Status = 'PTA',
					SR.Approval_Flow_Sequence,
				...
				QH.Item_Template = IC.Id_Item AND IC.Status = 1		

				INNER JOIN Cat_Extended_Warranties EW ON
				IC.Id_Line = EW.Id_Line AND
				QH.Id_Year_Warranty = EW.Id_Year_Warranty

				INNER JOIN Approval_Tracker  APT ON   
				EW.Id_Approval_Flow = APT.Id_Approval_Flow

				INNER JOIN Security_Roles SR ON 
				APT.Id_Role = SR.Id_Role

				WHERE 
				QH.Folio = @piFolio AND 
				QH.[Version] = @piVersion
